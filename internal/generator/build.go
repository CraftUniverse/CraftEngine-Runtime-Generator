...
func buildTarget(version, goos, goarch, ext, outDir string) {
	...
	outputName := filepath.Join(outDir, fmt.Sprintf("generator-%s-%s%s", goos, goarch, ext))

	// Run build command
	cmd := exec.Command("go", "build", "-ldflags", "-w -X 'main.version="+version+"'", "-o", outputName, "./cmd/generator/main.go")
	cmd.Env = env
